...
public class TipoProductoResource {
    ...
    @GET
    @Path("{idTipo}/productos")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getProductosConPreciosByTipo(
            @PathParam("idTipo") Long idTipoProducto) {

        if (idTipoProducto == null || idTipoProducto <= 0) {
            LOGGER.log(Level.WARNING, "ID de tipo producto inválido: {0}", idTipoProducto);
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity("ID de tipo producto inválido")
                    .build();
        }

        try {
            List<Object[]> resultados = tpBean.findProductsInTipoProducto(idTipoProducto);

            if (resultados.isEmpty()) {
                return Response.status(Response.Status.NOT_FOUND)
                        .entity("No se encontraron productos para el tipo especificado")
                        .build();
            }

            // Convertir Object[] a una lista de Map
            List<Map<String, Object>> responseData = new ArrayList<>();

            for (Object[] fila : resultados) {
                Map<String, Object> item = new HashMap<>();
                item.put("idProducto", fila[0]);    // Long
                item.put("nombre", fila[1]);        // String
                item.put("precioSugerido", fila[2]); // BigDecimal
                item.put("observaciones", fila[3]);  // String
                item.put("activo", fila[4]);         // Boolean 
            ...
            LOGGER.log(Level.SEVERE, "Error al obtener productos con precios: {0}", e.getMessage());
            return Response.serverError()
